...
class FixtureDriver<DomainT, NoFixture, void (*)(Args...), SeedProvider>
 ...
 public:
  ...
  void Test(MoveOnlyAny&& args_untyped) const override {
    std::apply( // 函数主要功能是使用std::apply函数将args_untyped参数包装成value_type_t<DomainT>类型，并通过解包传递给target_function_函数。
        [&](auto&&... args) {
          target_function_(ForceVectorForStringView<Args>(std::move(args))...); // 其中，target_function_是一个函数对象（可能是lambda表达式或其它类型的函数对象），它以Args类型参数 pack 的形式接受输入参数。
        },
        args_untyped.GetAs<value_type_t<DomainT>>());
  }
